import Link from "next/link";

const TermServiceContent = () => {
  return (
    <div>
      <h1>Terms of Service</h1>
      <p>
        <strong>Last Updated: April 17, 2026</strong>
      </p>
      <p>
        Welcome to NetProxy.io (&quot;NetProxy.io,&quot; &quot;we,&quot; &quot;us,&quot; or
        &quot;our&quot;). These Terms of Service (&quot;Terms&quot;) govern your
        access to and use of our proxy and related services, including all
        associated applications, software, websites, products, and content that
        we make available (collectively, the &quot;Services&quot;).
      </p>
      <p>
        By accessing or using the Services, you affirm that you have read,
        understood, and agree to be legally bound by these Terms, as well as our
        Privacy Policy and Cookie Policy. If you do not agree to these Terms,
        you must discontinue your use of the Services immediately.
      </p>
      <h2>1. Eligibility and Account Registration</h2>
      <p>
        1.1 <strong>Eligibility</strong>: You must be at least 18 years of age,
        or the age of majority in your jurisdiction (if different), to use the
        Services. By using the Services, you represent and warrant that you meet
        this eligibility requirement.
      </p>
      <p>
        1.2 <strong>Account Creation</strong>: Certain features of the Services
        may require you to create an account. When creating an account, you
        agree to provide accurate, current, and complete information, and to
        promptly update such information should it change. You are responsible
        for safeguarding your account credentials and for all activities that
        occur under your account.
      </p>
      <p>
        1.3 <strong>Prohibited Account Activities</strong>: You may not
        impersonate another person, create an account using a false identity, or
        register for an account on behalf of someone else without authorization.
      </p>
      <h2>2. License and Restrictions</h2>
      <p>
        2.1 <strong>Limited License</strong>: Subject to your compliance with
        these Terms, we grant you a limited, non-exclusive, non-transferable,
        and revocable license to access and use the Services solely for your
        personal, non-commercial use.
      </p>
      <p>
        2.2 <strong>Prohibited Uses</strong>: You agree not to:
      </p>
      <ul>
        <li>
          <p>
            Use the Services for any illegal, fraudulent, or unauthorized
            purposes;
          </p>
        </li>
        <li>
          <p>
            Engage in activities that violate any applicable law, regulation, or
            the rights of third parties;
          </p>
        </li>
        <li>
          <p>
            Attempt to gain unauthorized access to our systems, reverse-engineer
            any part of the Services, or introduce malware;
          </p>
        </li>
        <li>
          <p>
            Interfere with or disrupt the integrity or performance of the
            Services;
          </p>
        </li>
        <li>
          <p>
            Resell, redistribute, sublicense, or otherwise exploit the Services
            for commercial purposes without our express written permission.
          </p>
        </li>
      </ul>
      <p>
        2.3 <strong>Compliance</strong>: You must comply with all applicable
        laws and regulations when using the Services, including intellectual
        property, privacy, and data protection laws.
      </p>
      <h2>3. P2P Usage of Your Network Resources</h2>
      <p>
        3.1 <strong>Acknowledgment</strong>: By using the Services, you
        acknowledge and agree that a portion of your device’s internet bandwidth
        and network resources may be used to route traffic to and from other
        users on a peer-to-peer (P2P) basis.
      </p>
      <p>
        3.2 <strong>Purpose</strong>: This P2P functionality enhances the
        performance, resilience, and anonymity of our proxy network.
      </p>
      <p>
        3.3 <strong>Opt-Out</strong>: If you do not consent to the use of your
        network resources for P2P traffic, you must discontinue using the
        Services immediately.
      </p>
      <h2>4. Intellectual Property</h2>
      <p>
        4.1 <strong>Ownership</strong>: All content, trademarks, service marks,
        logos, and intellectual property displayed on or through the Services
        are the exclusive property of NetProxy.io or its licensors. Nothing in these
        Terms grants you any right, title, or interest in or to any of our
        intellectual property.
      </p>
      <p>
        4.2 <strong>User Feedback</strong>: If you provide suggestions, ideas,
        or feedback, you grant us a worldwide, perpetual, irrevocable,
        royalty-free license to use, reproduce, modify, and incorporate your
        feedback into the Services or other products without any obligation to
        you.
      </p>
      <h2>5. Payments, Fees, and Taxes</h2>
      <p>
        5.1 <strong>Subscriptions and Payments</strong>: If the Services or
        certain features require payment, you agree to pay all applicable fees.
        We may use third-party payment processors, and your use of their
        services may be subject to their terms.
      </p>
      <p>
        5.2 <strong>Billing Information</strong>: You must provide current,
        accurate, and complete billing information. By providing payment
        details, you authorize us to charge the applicable fees to the
        designated payment method.
      </p>
      <p>
        5.3 <strong>Refunds</strong>: Unless otherwise stated, all fees are
        non-refundable. We may, at our sole discretion, issue refunds or credits
        in certain cases.
      </p>
      <p>
        5.4 <strong>Taxes</strong>: You are responsible for paying any
        applicable taxes (if any) associated with your use of the Services.
      </p>
      <h2>6. Termination and Suspension</h2>
      <p>
        6.1 <strong>Termination by You</strong>: You may discontinue using the
        Services at any time. If you wish to delete your account, please contact
        us.
      </p>
      <p>
        6.2 <strong>Termination by Us</strong>: We reserve the right to suspend,
        limit, or terminate your access to the Services at any time, with or
        without notice, if we believe you have violated these Terms or engaged
        in any unlawful or harmful activity.
      </p>
      <p>
        6.3 <strong>Effect of Termination</strong>: Upon termination, the
        license granted to you under these Terms ends, and you must cease all
        use of the Services. Sections that by their nature should survive
        termination (e.g., intellectual property, disclaimers, limitation of
        liability, governing law, dispute resolution) will remain in effect.
      </p>
      <h2>7. Disclaimers</h2>
      <p>
        7.1 <strong>As-Is Basis</strong>: The Services are provided &quot;as
        is&quot; and &quot;as available,&quot; without warranties of any kind,
        whether express, implied, or statutory. We disclaim all warranties,
        including warranties of merchantability, fitness for a particular
        purpose, title, and non-infringement.
      </p>
      <p>
        7.2 <strong>No Guarantee of Availability</strong>: We do not warrant
        that the Services will be uninterrupted, secure, or error-free, or that
        any defects will be corrected. We assume no responsibility for any loss
        or damage resulting from downtime, data loss, or the unavailability of
        the Services.
      </p>
      <h2>8. Limitation of Liability</h2>
      <p>
        8.1 <strong>Limitations</strong>: To the fullest extent permitted by
        law, in no event shall NetProxy.io, its affiliates, partners, or licensors be
        liable for any indirect, incidental, consequential, special, exemplary,
        or punitive damages, including lost profits, data loss, or service
        interruption, arising out of or related to your use of or inability to
        use the Services.
      </p>
      <p>
        8.2 <strong>Aggregate Liability</strong>: In no event will our total
        liability to you exceed the total amount paid by you for the Services in
        the twelve (12) months preceding the event giving rise to the claim, or
        one hundred US dollars (USD 100) if no fees were paid.
      </p>
      <h2>9. Indemnification</h2>
      <p>
        You agree to indemnify, defend, and hold harmless NetProxy.io and its
        affiliates from and against any claims, losses, liabilities, damages,
        costs, and expenses (including reasonable attorneys’ fees) arising out
        of or related to:
      </p>
      <ul>
        <li>
          <p>Your breach of these Terms;</p>
        </li>
        <li>
          <p>Your misuse of the Services;</p>
        </li>
        <li>
          <p>
            Your violation of applicable laws or the rights of any third party.
          </p>
        </li>
      </ul>
      <h2>10. Governing Law and Dispute Resolution</h2>
      <p>
        10.1 <strong>Governing Law</strong>: These Terms and any dispute or
        claim arising out of or related to them shall be governed by and
        construed in accordance with the laws of the jurisdiction where NetProxy.io is
        incorporated, without regard to conflict-of-law principles.
      </p>
      <p>
        10.2 <strong>Arbitration</strong>: Except where prohibited by law, you
        agree that any disputes arising out of or related to these Terms or the
        Services shall be resolved through final and binding arbitration,
        conducted by a mutually agreed-upon arbitrator in accordance with
        applicable arbitration rules. Arbitration shall take place in the
        jurisdiction of our choice, and the language of the arbitration shall be
        English.
      </p>
      <p>
        10.3 <strong>Class Action Waiver</strong>: You agree to resolve any
        disputes on an individual basis and waive any right to bring or
        participate in a class action lawsuit or class-wide arbitration.
      </p>
      <h2>11. Changes to These Terms</h2>
      <p>
        We may update these Terms from time to time. The &quot;Last
        Updated&quot; date at the top indicates the most recent changes. By
        continuing to use the Services after any changes become effective, you
        agree to be bound by the revised Terms.
      </p>
      <h2>12. Miscellaneous</h2>
      <p>
        12.1 <strong>Entire Agreement</strong>: These Terms, together with our
        Privacy Policy and Cookie Policy, constitute the entire agreement
        between you and NetProxy.io regarding your use of the Services, superseding any
        prior agreements.
      </p>
      <p>
        12.2 <strong>Severability</strong>: If any provision of these Terms is
        found to be invalid or unenforceable, the remaining provisions shall
        remain in full force and effect.
      </p>
      <p>
        12.3 <strong>No Waiver</strong>: Our failure to enforce any right or
        provision of these Terms does not constitute a waiver of such right or
        provision.
      </p>
      <p>
        12.4 <strong>Assignment</strong>: You may not assign or transfer your
        rights or obligations under these Terms without our prior written
        consent. We may assign or transfer our rights and obligations freely.
      </p>
      <h2>13. Contact Us</h2>
      <p>
        If you have questions or concerns about these Terms, please contact us
        at:
      </p>
      <p>
        <strong>NetProxy.io Legal Department</strong>
        <br />
        Website:
        <Link href="/">https://netproxy.io</Link>
      </p>
      <p>© 2026 NetProxy.io. All rights reserved.</p>
      <p>
        <br />
        <br />
      </p>
      <p>Copyright © NetProxy.io. All rights belong to NETPROXY.IO</p>
    </div>
  );
};
export default TermServiceContent;
